...
数字宣言の次に出す本数を選び、手のSVGまたは本数ボタンへ即時反映します。公開中は中央の表示をカウント・合計・成否へ切り替え、操作中の二重処理は状態機械と入力ロックで防ぎます。320×568、375×667、390×844、430×932のモバイル幅を確認済みです。
...
src/
  components/
    ...
    HandGraphic.tsx      左右の手と0/1/2本の親指SVG
...
手、親指、キャラクター、背景表現、アイコン、パーティクルはすべてこのリポジトリのTypeScript/JSX/CSS/SVGで作成したオリジナルです。外部画像、外部フォント、外部音源、権利不明の効果音は使用していません。効果音と低音BGMはWeb Audio APIで波形を生成し、初回アクセス時の自動再生は行いません。
