...
キャラクター、背景表現、アイコン、パーティクルはリポジトリ内のTypeScript/JSX/CSS/SVGで作成しています。手画像はユーザー提供画像から切り出した`public/assets/hands/*.png`を使用します。元画像の公開利用許諾・ライセンス確認は公開者の責任で行ってください。外部フォント、外部音源、権利不明の効果音は使用していません。効果音と低音BGMはWeb Audio APIで波形を生成し、初回アクセス時の自動再生は行いません。
