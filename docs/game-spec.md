...
手は完成済みの右手画像アセット（`fist-right.png`・`thumb-up-right.png`）を状態に応じて切り替え、左手は同じ画像を水平反転します。左右別の手画像や手の輪郭をコードで生成しません。キャラクターSVGは共通の顔骨格に、髪、服、装具、表情差分をキャラクター別に合成します。状態クラスで構え、公開、成功、失敗、脱落を表現します。
